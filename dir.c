		    ...
		    vm_size_t bufsiz, int *amt)
{
  ...
  realsiz = round_page((char *) entp - *data);
  if (mapped && bufsiz > realsiz) {
    munmap(*data + realsiz, bufsiz - realsiz);
  }
		      ...
		      struct protid *cred)
{
  ...
  struct anonfs_dirent *new;

  if (round_page (anonfs_space_used + entsize) / vm_page_size
      > anonfs_page_limit)
    return ENOSPC;

  new = malloc (offsetof (struct anonfs_dirent, name) + namelen + 1);
  if (new == 0)
    return ENOSPC;
...
{
  struct anonfs_dirent *d = *ds->prevp;
  const size_t entsize
